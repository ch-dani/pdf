...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	//TODO кастомная аннотация
	function CustomAnnotation($x, $y, $w, $h, $text="annotation",$depr = false, $content="content", $title="title", $color=false, $type="Highlight")
	{
	
	
		$l = [$x * Mpdf::SCALE, $this->hPt - $y * Mpdf::SCALE, $w * Mpdf::SCALE, $h * Mpdf::SCALE, false, false, $content, $title, $color, $type];
	...
	function watermark2($texte, $angle = 0, $fontsize = 96, $alpha = 0.2, $font="serif", $top=0, $left=0)
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
		...
		$this->SetFont($font, "B", $fontsize, false); // Don't output
		$texte = $this->purify_utf8_text($texte);
	...
	function ImportPage($pageno = 1, $crop_x = null, $crop_y = null, $crop_w = 0, $crop_h = 0, $boxName = '/CropBox', $deleted_elements=array())
	{
		$fn     = $this->current_filename;
...
//		$deviations = "2";
//		$deviations2 = "3";
//		$this->tpls[$this->tpl]['buffer'] = str_replace("( )Tj","(1)Tj", $this->tpls[$this->tpl]['buffer']);
//		$this->tpls[$this->tpl]['buffer'] = str_replace("( )'", "(2)'", $this->tpls[$this->tpl]['buffer']);		

		
		$x = new \Mpdf\ReplaceMetrics(); //hyerga
		
		$replaced_data = $x->replace($deleted_elements, $this->tpls[$this->tpl]['buffer'], $this->h, $this->w, Mpdf::SCALE);
		
		$this->tpls[$this->tpl]['buffer'] = $replaced_data['buffer'];
...
//		
//		//buffer
//		var_dump($this->tpls[1]);
//		exit("sss");
		

		return ['tplId'=>$this->tpl, "not_replaced_elements"=>$replaced_data['not_replaced_elements'], "rotated"=>$rotated, "tpl_box"=>$tpl['box']];
	}
	...
	function UseTemplate($tplidx, $_x = null, $_y = null, $_w = 0, $_h = 0)
	{
		...
		$_w = $wh['w'];
		$_h = $wh['h'];
		$out .= sprintf("q %.4F 0 0 %.4F %.2F %.2F cm", 
			($_w / $this->tpls[$tplidx]['box']['w']), 
			($_h / $this->tpls[$tplidx]['box']['h']), 
			$_x * Mpdf::SCALE, 
			($this->h - ($_y + $_h)) * Mpdf::SCALE) . "\n";
...
class PDFHeaderFooter extends \Mpdf\Mpdf{
	...
	public function htext($color="#ebebeb", $pos = "hcenter", $texte, $angle = 45, $fontfamily="Courier", $fontsize = 96, $alpha = 0.5, $top=false, $left=false, $rx=0, $ry=0){
		if ($this->PDFA || $this->PDFX) {
		...
		$temp_wx = ($this->w / 2) - $adj + $offset / 3;
		$temp_wy = ($fontsize/$scale)+3; //
...
//		$wy = ($fontsize/$scale); // 		
		...
		if($pos=='cord' and $rx and $ry){
			$this->Rotate($angle, $rx, $ry);
		}else{
			$this->Rotate($angle, $wx, $wy);
		}
		$this->Text($wx, $wy, $texte, $OTLdata, $textvar, "", "test text");
		$this->Rotate(0);
		$this->SetTColor("");
		
		return array("wx"=>$wx, "wy"=>$wy, "adj"=>$adj, "opp"=>$opp, "temp_wx"=>$temp_wx, "temp_wy"=>$temp_wy);
