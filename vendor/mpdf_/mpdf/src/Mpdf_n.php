...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		$zproperties = [];
		if (false && $mode === HTMLParserMode::DEFAULT_MODE || $mode === HTMLParserMode::HEADER_CSS) {
		...
		for ($i = 0; $i < $cnt; $i++) {
			...
			if ($i % 2 == 0) {
				...
				if (isset($e[0]) && $e[0] == '/') {
					...
					if (false && $this->allow_html_optional_endtags && !$parseonly) {
					...
					// mPDF 6
					if (false && $this->blk[$this->blklvl]['hide']) {
						if (in_array($endtag, $this->outerblocktags) || in_array($endtag, $this->innerblocktags)) {
					...
					$orig_srcpath = '';
					if (false && (stristr($e, "href=") !== false) or ( stristr($e, "src=") !== false)) {
		...
		if ($close) {
			...
			if (false && isset($this->blk[$this->blklvl]['float_endpos']) && $this->blk[$this->blklvl]['float_endpos'] > $currpos) {
				$old_page = $this->page;
			...
			if (false && !empty($this->internallink)) {
			...
			/* -- CSS-POSITION -- */
			if (false && ount($this->fixedPosBlockSave)) {
