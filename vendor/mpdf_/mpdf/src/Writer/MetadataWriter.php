...
//							$annotcolor .= ']';
							...
							$color_array= explode(" ",$pl['opt']['c']);
							
							$annotcolor = sprintf('%.3F %.3F %.3F', $color_array[0]/255, $color_array[1]/ 255, $color_array[2]/ 255);
							

							$rr = explode(" ", $rect);
							$text = $this->writer->utf16BigEndianTextString($pl[4]);
//							var_dump($text);
//							exit();
							$text = ($this->writer->string($text));
//							exit();
							
							
							
							if($pl[6]){
//								var_dump(utf8_encode($pl[6]));
								$pl[6] = $this->writer->utf16BigEndianTextString($pl[6]);
								//$pl[6] = (utf8_decode($pl[6]));
							...
							if($pl[7]){
								$pl[7] = $this->writer->utf16BigEndianTextString($pl[7]);
								$tit = "/T {$pl[7]} ";
							}else{
								$tit = "";
							}
							...
							$annot = "<< /C [ $annotcolor] /CA 0.5 /Border [0 0 0] $tit $cont  /QuadPoints [ {$rr[0]} {$rr[3]} {$rr[2]} {$rr[3]} {$rr[0]} {$rr[1]} {$rr[2]} {$rr[1]} ] /Rect [ $rect ] /Subtype /{$pl[9]} /Type /Annot >>";
