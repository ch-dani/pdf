...
class PDFHeaderFooter extends \Mpdf\Mpdf{
	...
	public function htext2($styles=[],$color="#ebebeb", $pos = "hcenter", $texte, $angle = 45, $fontfamily="Courier", $fontsize = 96, $alpha = 0.5, $top=false, $left=false, $rx=0, $ry=0){
		if ($this->PDFA || $this->PDFX) {
		...
		//trans
		$this->SetFont($fontfamily, $font_style, $fontsize, false); // Don't output
		$texte = $this->purify_utf8_text($texte);
		if ($this->text_input_as_HTML) {
			$texte = $this->all_entities_to_utf8($texte);
		}
		...
		$maxlen = (min($this->w, $this->h)); // sets max length of text as 7/8 width/height of page
		while ($loop == 0) {
			$this->SetFont($fontfamily, $font_style, $szfont, false); // Don't output
		...
		$this->SetFont($fontfamily, $font_style, $szfont - 0.1, true, true); // Output The -0.1 is because SetFont above is not written to PDF
