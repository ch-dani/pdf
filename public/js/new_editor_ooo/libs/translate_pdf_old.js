...
$(document).on("translate_proggress", function(e, percent, translated_count){
	if(percent>=100){
		$(".translate_percent_progress").html("Create preview...");
	}else{
...
async function createPdfOnServer(){
	...
	var pages_content = {};
	var props = ["width", "height", "left", "top", "text", "fontSize", "unicode_string"];

	$.each(viewer.pages, (i,page)=>{
		page_json = page.fcanvas.toObject(props, true);
	...
	fillPDF(e, texts){
		...
		$.each(texts, (pn, page)=>{
			...
			$.each(page, (tn, text)=>{
				...
				//heig
				var fsFlag = false;
				if(textbox.width>textbox.fixedWidth*0.96){
					fsFlag = true;
					textbox.originalFontSize = textbox.fontSize;
					textbox.fontSize *= textbox.fixedWidth/(textbox.width+1);
				}

				// if(textbox.width<textbox.fixedWidth*0.96 && textbox.height>textbox.fixedHeight*0.96){
				// 	textbox.fontSize *= textbox.fixedHeight/(textbox.height+1);
				// }
