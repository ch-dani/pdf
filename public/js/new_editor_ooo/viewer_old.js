...
//	};
	...
	this.afterPageRender = async (pageNumber, page_object) => {
		...
		var total_text_rendered = 0;
		$(document).trigger("after_page_render", [pageNumber, page_object])
		var canvas = this.pages[pageNumber].canvas,
		...
		//renderRect(this.pages[pageNumber].fabricRects, fcanvas, pageNumber);
...
//							string += "\n";
			...
			//itext.on("mousedown", textFunc.elementClick);
...
//			if((total_text_rendered >12 && total_text_rendered <15 )|| (total_text_rendered>22 && total_text_rendered<25)){
				fcanvas.add(itext);	
				//fcanvas.moveTo(itext, zIndex.text);
//			}
			total_text_rendered++;
			//return false;
		});
		...
		//renderImages(viewer.pages[pageNumber].fabricImages, fcanvas,pageNumber);
...
//		console.log("page_context is", page_context);
		...
		var pn = await page.render({canvasContext: page_context,  viewport: viewport });
		// page.cleanup();
		delete viewport;
		//delete page;
...
fabric.IText.prototype.onKeyUp = (function(onKeyUp) {
	return function(e){
		...
		var printable = 
			...
			keycode == 32 || keycode == 13  || keycode==173  || // spacebar & return key(s) (if you want to allow carriage returns)
...
//			console.log("=== save ===", viewer.pages[pageNumber].trans_stack);
		...
		case 31: //start text
			...
			// viewer.pages[pageNumber].line_offset_top = 0; //TODO под вопросом
			// viewer.pages[pageNumber].append_to_pos = 0;
			
			// viewer.pages[pageNumber].first_in_block = true; 
			// viewer.pages[pageNumber].block_left = 0;
			// viewer.pages[pageNumber].block_top = 0;
...
//		}
		...
		$.each(block, function(ln, line){
			...
			if(!prev_line){
				...
				// console.log(parseInt(prev_line.right+prev_line.spaceWidth)>= parseInt(current_line.left));
				if(prev_line.spaceWidth<1){
					prev_line.spaceWidth = 3;
				}

				if(!stopMerge &&
					//toInt(current_line.spaceWidth) == toInt(prev_line.spaceWidth) 
					current_line.left > prev_line.left &&
					//prev_line.right < current_line.left &&

					(parseInt(prev_line.right+prev_line.spaceWidth)>= parseInt(current_line.left)) 
					//&& toInt(current_line.top)==toInt(prev_line.top)
					&& toInt(current_line.bottom)==toInt(prev_line.bottom)
					//&& (prev_line.height >= current_line.height)
				){
					cl = new_blocks[block_iterator].getCurrentLine(); 

					// if(pageNumber==1){
					// 	console.log("================================");
					// 	// console.log(current_line.left, prev_line.left);
					// 	console.log(prev_line.getPlainText());
					// 	 console.log(current_line.getPlainText());
					// 	console.log(prev_line.right, current_line.left);
					// 	console.log(prev_line);

					// 	alert("add to line "+pageNumber);
					// }
