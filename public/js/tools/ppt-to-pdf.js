...
//         PPT2PDF.file_name = file.name;
//         // $(document).on("file_selected", this.save);
//         $(".r_upload_section, .module__how-convert, .module__about-tools, .module__our-blog, .module__how-banner, .module__testimonial").hide();
//         $("#ppt_section").removeClass("hidden");
//
//         let $tmp_canvas = $(
//             '<div class="convert_doc right_doc image-canvas-li">' +
...
// }
//
// if (PPT2PDF.tool_section.length > 0) {
//     PPT2PDF = $.extend(PDFTOOLS, PPT2PDF);
//     PPT2PDF.main();
// }

var A_TOOL = false;
var ALLOW_FILE_EXT = "application/powerpoint";
var number = 0;
FILE_NUM = 0;
    ...
    var toolJpgToPdf = {
            ...
            fileSelected: function (e, file_obj = false) {
                ...
                if (file_ext != 'ppt' && file_ext != 'pptx') {
                    swal("Error", "Please select PPT or PPTX file.", "error");
                } else {
                    Array.from(files).forEach(file => {
                        that.getBase64(file).then(function (base64) {
                            ...
                            let $tmp_canvas = $(
                                ...
                                '<div class="convert_doc_content">' +
                                '<div class="download_convert_doc">' +
                                '<img src="https://w0.pngwave.com/png/292/415/microsoft-powerpoint-computer-icons-presentation-slide-presentation-png-clip-art-thumbnail.png" alt="">' +
                                '</div>' +
                                '</div>' +
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            if (data.status == 'success') {
                                                ...
                                                $("#save-gdrive").attr({
                                                    'data-src': location.protocol + '//' + location.hostname + data.url,
                                                    'data-filename': data.new_file_name,
                                                    'data-sitename': 'Freeconvert PDF'
                                                });
            ...
            uploadFile: function (files) {
                ...
                formData.append("path", 'ppt-to-pdf');
