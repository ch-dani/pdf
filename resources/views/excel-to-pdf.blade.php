...
@section('content-freeconvert')
    <main class="file_not_loaded">
            ...
            <div class="container">
                <div class="title-wrapper">
                    <h2 class="h30-title title_main">
                        {!! array_key_exists(1, $PageBlocks) ? $PageBlocks[1] : t("Convert Excel to PDF Documents (XLS to PDF)") !!}
                    </h2>
                    <h3 class="sub-title">
                        {!! array_key_exists(2, $PageBlocks) ? $PageBlocks[2] : t("Creates a PDF document from Microsoft Excel file (.xls)") !!}
                    </h3>
                </div>
        ...
        {{--        @include('inc-freeconvert.how_to_convert')--}}
        <section class="module__how-convert module bg-white">
            <div class="container">
                <div class="title-wrapper">
                    <h2 class="h2-title title_main">{{ t("How to convert Excel to PDF?") }}</h2>
                </div>
                <div class="row">
                    @if (count($PageGuides))
