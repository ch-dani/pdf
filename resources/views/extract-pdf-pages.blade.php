...
@section('content')
	...
	<section class="new-edit-example hidden split_section" >
		<div class="app-title">
			...
			<div class="edit-message">{!! t('Selected:') !!} <span data-value="filename">%file_name%</span></div>
    ...
    <section class="split-page-idit preview_section hidden" id="pages_preview_section">
        <div class="split-page-top">
            <p>{!! t('Click pages to select.') !!}</p>
            <a class="button-green clear-btn" id="reset_selected_pages" href="#">{!! t('Reset selection') !!}</a>
	...
	<section class="fixed-bottom-panel hidden" id="pages_ranges">
        <form class="fixed-task-form">
            <div class="image-radio-item">
                <span class="btns-resolution-span">{!! t('Selected pages:') !!}</span>
                <div class="btns-resolution">
                    <label class="resolution-item">
                        ...
                        <span class="resolution-item-checkmark">
                            ...
                            <span class="input-help tooltip">
                                <span class="tooltiptext">{!! array_key_exists(4, $PageBlocks) ? $PageBlocks[4] : 'Get a new document containing only the odd pages 1,3,5,7 etc from the original.' !!}</span>
                    ...
                    <label class="resolution-item">
                        ...
                        <span class="resolution-item-checkmark">
                            {!! t('All Even Pages') !!}
                            <span class="input-help tooltip">
                                <span class="tooltiptext">{!! array_key_exists(5, $PageBlocks) ? $PageBlocks[5] : 'Get a new document containing only the even pages 2,4,6,8, etc from the original.' !!}</span>
                                <i class="far fa-question-circle"></i>
                            </span>
                        </span>
                    </label>
            ...
            <div class="more-options-box">
                <div class="checkbox-item-content">
                    <label>
                        ...
                        <span>{!! t('Discard bookmarks') !!}</span>
                    </label>
                    <span class="input-help tooltip">
                        <span class="tooltiptext">{!! array_key_exists(6, $PageBlocks) ? $PageBlocks[6] : 'Remove bookmarks and outline from the result.' !!}</span>
                        <i class="far fa-question-circle"></i>
                    </span>
                </div>
            </div>
            <div class="more-options-btns-wrap">
                <button class="options-btn" type="button" id="start_task">{!! t('Extract pages') !!}</button>
